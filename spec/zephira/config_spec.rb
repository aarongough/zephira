...
RSpec.describe Zephira::Config do
  ...
  describe ".read" do
    ...
    def create_project_config(key: "ZEPHIRA_TEST_KEY", value: "project_key")
      File.write(".zephira.yml", "#{key}: #{value}\n", mode: "a")
    end

    def create_global_config(key: "ZEPHIRA_TEST_KEY", value: "global_key")
      FileUtils.mkdir_p(File.expand_path("~"))
      File.write(File.expand_path("~/.zephira.yml"), "#{key}: #{value}\n", mode: "a")
    end
    ...
    context "when the project config exists but does not contain the key and the global config does" do
      it "returns the value from the global config file" do
        create_project_config(key: "OTHER_KEY")
        create_global_config
    ...
    context "when the environment variable is set to an empty string" do
      it "returns the empty string (and does not fall back to config files)" do
        ENV["ZEPHIRA_TEST_KEY"] = ""
        create_global_config
        create_project_config

        expect(described_class.read("ZEPHIRA_TEST_KEY")).to eq("")
      end
    end
